# Production image for @x1agent/provider-graph-surrealdb. Multi-stage,
# same shape as api.prod.Dockerfile so the build pipeline doesn't have
# to special-case it. Provider speaks NATS (no HTTP port to expose);
# liveness comes from the NATS connection itself.
...
# Provider needs the kernel + shared types + cross-domain port packages
# its index.ts imports. Skip api/app/cli — not reachable.
...
COPY --chown=agent:agent packages/providers/graph-surrealdb/ ./packages/providers/graph-surrealdb/
