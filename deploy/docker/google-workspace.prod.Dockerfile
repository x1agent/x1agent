# Production image for @x1agent/provider-google-workspace. Multi-stage,
# same shape as graph-surrealdb.prod.Dockerfile so the build pipeline
# doesn't have to special-case it. Provider speaks NATS (no HTTP port
# to expose); liveness comes from the NATS connection itself.
...
# Provider needs kernel + shared types + observability + the provider
# package's own source. Skip api/app/cli — not reachable.
...
COPY --chown=agent:agent packages/providers/google-workspace/ ./packages/providers/google-workspace/
