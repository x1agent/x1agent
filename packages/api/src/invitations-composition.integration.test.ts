...
// Wrapped in an outer describe so the expensive freshTestDb (DROP +
// CREATE DATABASE + 40 migrations) runs once for both inner describes.
// In bun 1.3+, top-level beforeAll fires once per sibling describe at
// the same scope — splitting the cost across describes pushed the
// per-describe budget past the default 5s on cold CI runners.
