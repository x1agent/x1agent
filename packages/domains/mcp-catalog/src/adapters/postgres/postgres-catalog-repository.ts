...
export class PostgresCatalogRepository implements CatalogRepository {
  ...
  async list(workspaceId: string): Promise<CatalogEntry[]> {
    const rows = await this.sql<CatalogRow[]>`
      SELECT id, workspace_id, name, display_name, image, manifest,
  ...
  async upsert(input: CatalogUpsertInput): Promise<CatalogEntry> {
    const [row] = await this.sql<CatalogRow[]>`
      INSERT INTO mcp_catalog_entries
        (workspace_id, name, display_name, image, manifest, description, created_by)
      VALUES (
        ${input.workspaceId},
      ...
      ON CONFLICT (workspace_id, name) DO UPDATE SET
        ...
        updated_at = now()
      RETURNING id, workspace_id, name, display_name, image, manifest,
                description, created_at, updated_at, created_by
    `;
