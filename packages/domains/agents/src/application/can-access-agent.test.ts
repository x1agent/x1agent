...
class FakeAgents {
  ...
  add(a: Agent) { this.byId.set(a.id, a); }
  async findById(id: AgentId) { return this.byId.get(id) ?? null; }
}

class FakeGrants {
  private rows: Array<{ agentId: string; subject: { kind: string; id: string | null }; verb: AgentVerb }> = [];
  add(agentId: string, subjectKind: string, subjectId: string | null, verb: AgentVerb) {
    this.rows.push({ agentId, subject: { kind: subjectKind, id: subjectId }, verb });
  }
  async listVerbsForResolver(input: {
    ...
    for (const r of this.rows) {
      ...
      if (
        ...
        (r.subject.kind === "group" && r.subject.id !== null && input.userGroupIds.includes(r.subject.id)) ||
  ...
  async upsert() { return null as unknown as AgentGrant; }
  async remove() {}
...
describe("resolveAgentAccess", () => {
  it("owner gets all three verbs", async () => {
    ...
    expect(r).toMatchObject({ canView: true, canInvoke: true, canEdit: true, via: "owner" });
  });

  it("workspace admin sees all even on a private agent", async () => {
    agents.add(makeAgent({ ownerUserId: ALICE, visibility: "private" }));
    const r = await resolveAgentAccess(deps as never, AGT, BOB, ctx({ isWorkspaceAdmin: true }));
    expect(r).toMatchObject({ canView: true, canInvoke: true, canEdit: true, via: "admin" });
  });

  it("private agent — non-owner non-admin sees nothing", async () => {
    agents.add(makeAgent({ ownerUserId: ALICE, visibility: "private" }));
    const r = await resolveAgentAccess(deps as never, AGT, BOB, ctx());
    expect(r).toMatchObject({ canView: false, canInvoke: false, canEdit: false, via: "denied" });
  });
  ...
  it("workspace-visible agent — any member can view + invoke; cannot edit", async () => {
    ...
    const r = await resolveAgentAccess(deps as never, AGT, BOB, ctx());
    expect(r).toMatchObject({ canView: true, canInvoke: true, canEdit: false, via: "visibility_workspace" });
  });
  ...
  it("via_grants — explicit user grant for invoke promotes view too", async () => {
    ...
    const r = await resolveAgentAccess(deps as never, AGT, BOB, ctx({ isWorkspaceMember: false }));
    expect(r).toMatchObject({ canView: true, canInvoke: true, canEdit: false, via: "grant" });
  });

  it("via_grants — group grant matches when user is in that group", async () => {
    agents.add(makeAgent({ ownerUserId: ALICE, visibility: "via_grants" }));
    grants.add(AGT, "group", "g-marketing", "invoke");
    const r = await resolveAgentAccess(deps as never, AGT, BOB, ctx({ userGroupIds: ["g-marketing"] }));
  ...
  it("via_grants — group grant does NOT match when user isn't in the group", async () => {
    agents.add(makeAgent({ ownerUserId: ALICE, visibility: "via_grants" }));
    grants.add(AGT, "group", "g-marketing", "invoke");
    const r = await resolveAgentAccess(deps as never, AGT, BOB, ctx({ userGroupIds: ["g-engineering"] }));
  ...
  it("edit verb is owner/admin only — visibility never grants edit", async () => {
    agents.add(makeAgent({ ownerUserId: ALICE, visibility: "workspace" }));
    expect(
      await userHasAgentVerb(deps as never, AGT, BOB, ctx(), "edit"),
    ).toBe(false);
  });
